...
def alpha_from_kappa(mapparams, kappa):
    # get lx, ly
    grid, _ = tools.make_grid(mapparams, Fourier = True)
    lx, ly = grid[0], grid[1]
    el2d = np.sqrt(lx**2. + ly**2.)
...
def lens_map(mapparams, unlensed_map, alpha_vec, centroid_shift = None):   
    # compute deflection field
    grid, _ = tools.make_grid(mapparams)  
    betaX, betaY = grid
...
class NFW:
    ...
    def kappa_map(self, mapparams):
        # getting kappa profile
        nx, dx, _, _ = mapparams
