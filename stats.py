...
def likelihood_function(data, models, mass_int, make_finer = 1):
    x, y, cov = data 
    ln_like = []
    ...
    L /= max(L)   
    if make_finer: 
        x_ip, L_ip = likelihood_finer_resol(mass_int, L) 
    else:
        x_ip = np.copy(mass_int)
        L_ip = np.copy(L)
    L_ip /= max(L_ip) 
    return x_ip, L_ip
...
def get_width_from_sampling(x, likelihood_curve, howmanysamples = 1000000, which_percentile = 16.):
    randoms = random_sampler(x, likelihood_curve, howmanysamples = howmanysamples)
    ...
    return mean_value, low_err, high_err
...
def run_mcmc(data, guess, offset, nwalkers, priors, nber_steps, burn_in):
    ...
    params = [(cred_int[i][0] + cred_int[i][1]) / 2 for i in range(len(priors))]
    errors = [params[i] - cred_int[i][0] for i in range(len(priors))]
    return chains, params, errors