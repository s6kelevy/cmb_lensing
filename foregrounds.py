...
def get_cl_ksz():
    ...
    cl_ksz[np.isnan(cl_tsz)] = 0.
    cl_ksz[np.isinf(cl_tsz)] = 0.
...
def extragalactic_power_spectrum(freq1, freq2 = None, components = None):
    ...
    if 'radio' in components:
        l, cl_radio = get_cl_radio(freq1, freq2)
        cl_extragal += cl_radio
    if 'cib' in components:   
        l,  cl_cib = get_cl_cib(freq1, freq2)
        cl_extragal += cl_cib
    if 'tsz' in components:   
        l,  cl_tsz = get_cl_tsz(freq1, freq2)
        cl_extragal += cl_tsz
    if 'ksz' in components:   
        l,  cl_ksz = get_cl_ksz()
        cl_extragal += cl_ksz    
    if 'tsz_cib' in components:   
        l,  cl_tsz_cib = get_cl_tsz_cib(freq1, freq2)
